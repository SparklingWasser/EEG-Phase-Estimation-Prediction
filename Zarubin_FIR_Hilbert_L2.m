...
filt_len_in_s = 0.1;
filt_order = floor(filt_len_in_s*fs) + mod(floor(filt_len_in_s*fs), 2);
B = fir1(filt_order, cutoff_freq/(fs/2));

data_filtered = filter(B, 1, data);   % zero-phase filter from literature
data_filtered_delay_corrected = data_filtered(filt_order/2+1:end);
t_delay_corrected = 0:1/fs:data_len - 1/fs - filt_len_in_s/2;
...
for i = 1:length(phase_shift)
    sin_waves(i, :) = cos(2*pi*target_freq*t_delay_corrected + phase_shift(i));
...
output = cos(2*pi*target_freq*t_prediction + phase_shift(min_idx));
